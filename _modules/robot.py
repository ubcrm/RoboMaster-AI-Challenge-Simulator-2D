...
from _modules.geometry import mirror, to_draw_coords, Line, Rectangle
...
def control_response(value, command, rate, maximum):
    value += 2 * command * rate
    return np.sign(value) * np.clip(np.abs(value) - rate, 0, maximum)
...
class Robot:
    ...
    def __init__(self, id_):
        ...
        self.can_shoot = True
        self.timeout = 0
        self.commands = np.zeros(5, dtype='int8')  # x, y, rotate, yaw, shoot
